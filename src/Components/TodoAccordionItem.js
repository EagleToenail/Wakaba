...
const TodoAccordionItem = ({time, title, content, fileUrl,sender,receiver, children ,parentMessageId,onSendData }) => {
      ...
      useEffect(() => {
        const fetchMessages = async () => {
            ...
            .then(response => {
                ...
                // Find sender and receiver names based on IDs
                users.forEach(user => {
          ...
          <div className='new-post-receive w-full flex h-13'>
            <div className='new-post-receive-message flex' style={{ width: '75%' }}>
              <div className='flex'>
                <div>
                  <div className='w-30'>
                    <label className="text-[black] font-bold text-[10px] block text-center">{formattedDate}</label>
                  </div>
            ...
            <div className='new-post-receive-name flex justify-between ' style={{ width: '25%' }}>
              <div className='flex'>
                ...
                <div>
                  <label className="text-[black] pl-3 text-[15px] block text-center">{receiverName || ''}</label>
                </div>
              </div>
              <div className='flex'>
                <div className='w-7 h-7 rounded-full border border-[#70685a] bg-[#70685a]'></div>
                <div>
                  <label className="text-[black] pl-3 text-[15px] block text-center">{senderName || ''}</label>
                </div>
              </div>
            </div>
          </div>
