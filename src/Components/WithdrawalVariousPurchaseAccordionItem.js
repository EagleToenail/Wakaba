      ...
      {isOpen && (
        <div style={styles.content}>
          ...
          {fileUrl && isImageFile(fileUrl) ? (<div className='w-full h-[150px] flex justify-center' >
            <div>
              <img src={`http://3.14.245.186:8081/api/uploads/withdrawalvariouspurchase/${fileUrl}`} alt="Uploaded" style={styles.imagePreview} />
              <a href={`http://3.14.245.186:8081/api/uploads/withdrawalvariouspurchase/${fileUrl}`} target="_blank">
                  Open in Brosewer 
              </a>
            </div>
          </div>
          ) : (<div className='w-full h-10 flex justify-center'>
            <a href={`http://3.14.245.186:8081/api/uploads/withdrawalvariouspurchase/${fileUrl}`} download={fileUrl} target="_blank" rel="noopener noreferrer">
                File Link : {fileUrl}
            </a>
          </div>
          )}
