  ...
  const handleGetParticipantsName = async (signal) => {
    try {
      const { data } = await axios.get(
        `/groups/${chatRoom.data.group.id}/participants/name`,
        { signal }
      );
  ...
  useEffect(() => {
    if (!isGroup) {
      ...
      socket.on('user/connect', (userId) => {
        if (userId === chatRoom.data.profile.user_id) {
          setSubhead('online');
    ...
    <nav className="h-16 grid grid-cols-[1fr_auto] gap-4 justify-between items-center bg-white dark:bg-spill-900">
      ...
      {/* {selectedChats && (
        <>
          ...
          {((isGroup &&
            chatRoom.data.group.participantsId.includes(master.id)) ||
              ...
              {[
                ...
                <button
                  ...
                  onClick={(e) => {
                    ...
                    if (
                      !isGroup ||
                      (isGroup && group.participantsId.includes(master.id))
                    ) {
                      elem.action();
                    }
                  }}
