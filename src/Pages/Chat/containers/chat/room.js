...
import RoomMoniter from '../../components/chat/room/monitor';
import RoomSend from '../../components/chat/room/send';
...
function Room() {
  ...
  const handleOpenRoom = async (signal) => {
    ...
    if (chatRoom.isOpen) {
      ...
      if (!isGroup || (isGroup && group.participantsId.includes(master.id))) {
